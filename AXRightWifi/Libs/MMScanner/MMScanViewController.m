...
- (void)initScanType{
    if (self.scanType == MMScanTypeAll) {
        ...
        _scanRect = CGRectFromString([self scanRectWithScale:1][0]);
        self.output.rectOfInterest = _scanRect;
        _tipTitle.text = @"将取景框对准二维码即可自动扫描";
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    ...
    if (metadataObjects.count>0) {
        ...
        NSString *result = @"";
        NSMutableString *muResult = [[NSMutableString alloc] initWithString:metadataObject.stringValue];
        if ([muResult containsString:@":"]) {
            
            result = [muResult componentsSeparatedByString:@":"].firstObject;
        } else {
...
//            [self renderUrlStr:[self configAddressWithStr:result withtype:self.popType]];
            return;
...
//绘制扫描区域
- (void)drawScanView {
    _scanRectView = [[MMScanView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_W, SCREEN_H) style:@""];
...
- (void)changeScanCodeType:(MMScanType)type {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        self->_tipTitle.text = type == MMScanTypeQrCode ? @"将取景框对准二维码即可自动扫描" : @"将取景框对准条码,即可自动扫描";
...
- (void)showError:(NSString*)str {
    [self showError:str andTitle:@"温馨提示"];
}
...
- (void)playAlertVoice{
    ...
    // 加载文件
    NSURL *fileURL = [NSURL fileURLWithPath:[[NSBundle mainBundle] pathForResource:@"tick" ofType:@"mp3"]];
