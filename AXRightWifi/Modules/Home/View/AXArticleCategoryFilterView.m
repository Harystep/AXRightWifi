...
#pragma mark - 视图
@interface AXArticleCategoryFilterView ()
...
@implementation AXArticleCategoryFilterView
...
//    self.containerView.backgroundColor = [UIColor brownColor];
    ...
    for (NSInteger i = 0; i < self.tagsArr.count; i ++) {
        ...
        label.tag = i;
        label.text = checkSafeContent(dict[@"title"]);
...
- (void)setContentView:(UILabel *)lb status:(BOOL)status {
    if (status) {
        lb.backgroundColor = [ZCConfigColor redColor];
